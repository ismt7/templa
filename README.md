...
- 本文に書く前でも、プレースホルダーだけを先に手動作成可能
- 手動作成時に任意のテキストエリアへ `{項目名}` をそのまま挿入可能
- 既存プレースホルダーを本文テキストエリアやモーダル編集画面からカーソル位置へ挿入可能
- テキスト選択中にプレースホルダーを挿入した場合は、選択範囲を `{項目名}` で置換
...
2. テンプレート名と本文を入力する
3. 必要なら「プレースホルダの値を入力」欄から、本文より先にプレースホルダーを追加する
4. 作成時に必要であれば、追加したプレースホルダーを任意のテキストエリアへ挿入する
5. 既存プレースホルダーを再利用する場合は、各テキストエリアまたはモーダル編集画面の「プレースホルダーを挿入」から差し込む
6. 本文内に `{項目名}` 形式でプレースホルダーを書く
7. テキストを選択した状態で挿入すると、その範囲が `{項目名}` に置き換わる
8. 必要に応じてプレースホルダー設定（テキスト/リスト/日付）を調整する
9. シーンごとに値を入力して、用途に応じて切り替える
10. コピー、または JSON エクスポートで利用する
